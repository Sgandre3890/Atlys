...
#define STB_IMAGE_IMPLEMENTATION // I'm using STB image instead of Assimp's built-in texture loading, because Assimp doesn't support flipping textures on load and that causes issues with OpenGL.
#include "stb_image.h"

struct Mesh {
    GLuint    vao        = 0; // These are buffer objects, used to store mesh data on the GPU, so that it runs really fast.
    GLuint    vbo        = 0;
    GLuint    ebo        = 0;
    int       indexCount = 0;
    GLuint    albedoTex  = 0;
...
static GLuint loadTexture(const aiString& aiPath, const aiScene* scene,
    ...
    const char* path = aiPath.C_Str(); // always C_Str(), this avoids the 4-char truncation bug that I was running into
