// Model Viewer
// WASD / arrows = move   Q/E = down/up   Left drag = look   Scroll = dolly
...
// Push all lighting uniforms to the bound shader
static void applyLighting(GLuint sh, const LightingSettings& L) {
    ...
    // Default to solid mode; wireframe/points passes override this
...
// A compact direction widget using two angle sliders that drives a vec3.
static void directionWidget(const char* label, float dir[3]) {
    // Convert current dir to angles
...
int main() {
    ...
    GLuint shader = createProgram("pbr.vert", "pbr.frag");
    ...
    double fpsTimer    = 0.0;
    int    fpsFrames   = 0;
    float  fpsDisplay  = 60.f;  // sensible seed so it shows immediately
    ...
    while (!glfwWindowShouldClose(win)) {
        ...
        if (ImGui::CollapsingHeader("Presets")) {
            ...
            if (ImGui::Button("Overcast",        ImVec2(-1,0))) {
                // Soft top-light, no strong directional, no specular
                lighting = LightingSettings{};
            ...
            if (ImGui::Button("Points",          ImVec2(-1,0))) {
                lighting = LightingSettings{};
                lighting.ambientColor[0]=1.f; lighting.ambientColor[1]=1.f; lighting.ambientColor[2]=1.f;
                lighting.ambientIntensity = 1.f;
                lighting.lights[0].intensity = 0.f;
        ...
        if (ImGui::CollapsingHeader("Performance")) {
            ...
            ImGui::TextDisabled("Tip: disabling culling shows inside faces");
            ImGui::TextDisabled("Tip: disable depth write for translucent meshes");
        }
        ...
        {
            // Use window size in logical points (not framebuffer pixels)
            // so it sits correctly on both Retina and normal displays
        ...
        if (modelLoaded) {
            ...
            if (renderMode == 0) {
                ...
                // Wireframe – flat color, no lighting
                ...
                // Solid + wire overlay
                glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
                applyLighting(shader, lighting);  // pass 1: solid
                drawModel(model, shader);
