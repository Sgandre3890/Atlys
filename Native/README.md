...
Camera, Shader, and Model are all classes with clear separation of public interface and private implementation — classic encapsulation. Camera has a private updateVectors() method that maintains internal state invariants, which is a textbook example of information hiding.
...
Model::destroy() manually calls glDeleteVertexArrays, glDeleteBuffers, and glDeleteTextures — manual resource cleanup, the C++ equivalent of what RAII and destructors formalize. This is a good example of why destructors exist.
...
Model::processNode() and Model::drawNode() are both recursive — they call themselves on child nodes to traverse the scene tree. This is a clean real-world example of tree traversal via recursion.

Pointers & References

Raw pointers used extensively in the accessor helpers (const unsigned char*) for reading binary buffer data with manual stride arithmetic — direct memory access.
...
enum class CameraMovement and enum class AppState — scoped enums that prevent accidental integer comparisons, a step up from plain #define constants.
...
The camera's updateVectors() converts spherical coordinates (yaw/pitch) to Cartesian using cos/sin — a direct application of trig.
